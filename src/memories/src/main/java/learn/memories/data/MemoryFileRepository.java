...
@Repository //@Repository annotation works just like @Component, but it provides extra context.
public class MemoryFileRepository implements MemoryRepository {

    private final String filePath;
    private final String delimiter = "~";
    /*
    public MemoryFileRepository(@Value("./data/memories.txt") String filePath) {
        this.filePath = filePath;
    }
    // @Value("./data/memories.txt") inserted before the argument in the constructor tells Spring annotations
    // to use this string as the argument.*/
    // The syntax ${dataFilePath} tries to fetch a value from a property source using the key `dataFilePath`.
    // if it doesn't find dataFilePath, then it will default to the string provided: ./data/default-file.txt
    public MemoryFileRepository(@Value("${dataFilePath:./data/default-file.txt}") String filePath) {
        this.filePath = filePath;
    }
